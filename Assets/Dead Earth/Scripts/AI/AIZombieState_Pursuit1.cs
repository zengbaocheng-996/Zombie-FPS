...
public class AIZombieState_Pursuit1 : AIZombieState
{
    ...
    public override AIStateType OnUpdate()
    {
        ...
        // Slowly update our rotation to match the nav agents desired rotation BUT only if we are not persuing the plaer and arrive
        else if (!_stateMachine.useRootRotation && !_zombieStateMachine.isTargetReached)
        {
            ...
            _zombieStateMachine.transform.rotation = Quaternion.Slerp(_zombieStateMachine.transform.rotation, newRot, Time.deltaTime);
        }
